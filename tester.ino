//0 for the motor is forward 180 is backwards, ~30 seconds to go from all the way back to all the way forward
...
const uint64_t TIME_DIVE = 10;//45;
uint64_t Time_of_Dive = 0;
const float POOL_DEPTH = 2.0;
...
float getDepth() {
    ...
    return 0;//(pressure / (density * gravity)) * 3.28084;
}

bool diveCompleted() {
    depth = getDepth();
    return true;//depth >= TARGET_DEPTH;
}

bool riseCompleted() {
    return true;//getDepth() <= 0;
}
